		...
		<div v-if="friends && friends.length > 0" class="friends-gallery_sub">
			...
			>
				...
				<button
					...
					@click="addFriendToUser(friend._id)"
				>
					Add
				</button>
				<button
...
export default {
	...
	methods: {
		addFriendToUser(friendId) {
			const myData = this.$store.getters.userData;
			const newFriendsList = { friends: [...myData.friends] };
			newFriendsList.friends.push(friendId);
